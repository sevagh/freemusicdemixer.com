...
static float find_max_abs(const Eigen::Tensor3dXcf &data, float scale_factor)
{
    ...
    for (int i = 0; i < 2; ++i)
    {
        for (int j = 0; j < data.dimension(1); ++j)
        {
            ...
            {
                max_val_im =
                    std::max(max_val_im, std::sqrt(std::norm(data(i, j, k))));
            }
        }
    }
...
// forward decl
static Eigen::Tensor4dXcf calculateCovariance(const Eigen::Tensor3dXcf &y_j,
                                              const int pos, const int t_end);

// Wiener filter function
std::array<Eigen::Tensor3dXcf, 4> umxcpp::wiener_filter(
    Eigen::Tensor3dXcf &mix_stft,
    const std::vector<Eigen::Tensor3dXf> &targets_mag_spectrograms)
{
    ...
    std::cout << "Wiener-EM: Getting first estimates from naive mix-phase"
              << std::endl;

    for (int target = 0; target < 4; ++target)
    {
        y[target] = umxcpp::polar_to_complex(targets_mag_spectrograms[target],
                                             mix_phase);
    }
    ...
    for (int i = 0; i < mix_stft.dimension(1); ++i)
    {
        ...
        {
            mix_stft(0, i, j) =
                std::complex<float>{mix_stft(0, i, j).real() / max_abs,
                                    mix_stft(0, i, j).imag() / max_abs};
            mix_stft(1, i, j) =
                std::complex<float>{mix_stft(1, i, j).real() / max_abs,
    ...
    for (int source = 0; source < 4; ++source)
    {
        for (int i = 0; i < mix_stft.dimension(1); ++i)
        {
            ...
            {
                y[source](0, i, j) =
                    std::complex<float>{y[source](0, i, j).real() / max_abs,
                                        y[source](0, i, j).imag() / max_abs};
                y[source](1, i, j) =
                    std::complex<float>{y[source](1, i, j).real() / max_abs,
    ...
    std::vector<Eigen::Tensor3dXcf>
        R; // A vector to hold each source's covariance matrix
    ...
    for (int it = 0; it < WIENER_ITERATIONS; ++it)
    {
        ...
        for (int frame = 0; frame < nb_frames; ++frame)
        {
            for (int bin = 0; bin < nb_bins; ++bin)
            {
                for (int source = 0; source < nb_sources; ++source)
                {
                    ...
                    // statistical summation, distributing v values for all
                    // frames
        ...
        for (int source = 0; source < nb_sources; ++source)
        {
            ...
            int pos = 0;
            int batchSize =
                WIENER_EM_BATCH_SIZE > 0 ? WIENER_EM_BATCH_SIZE : nb_frames;

            while (pos < nb_frames)
            {
                std::cout << "\tCovariance loop for source: " << source
                          << ", pos: " << pos << std::endl;
                int t_end = std::min(nb_frames, pos + batchSize);

                Eigen::Tensor4dXcf tempR =
                    calculateCovariance(y[source], pos, t_end);

                Eigen::Tensor3dXcf tempR3D(
                    tempR.dimension(1), tempR.dimension(2), tempR.dimension(3));
                tempR3D.setZero();
                ...
                // Update the weight summed v values across the frames for this
                // batch
        ...
        while (pos < nb_frames)
        {
            ...
            // Assuming you have a way to set all elements of y between frames
            // pos and t_end to 0.0
            for (int source = 0; source < 4; ++source)
            {
            ...
            Eigen::Tensor4dXcf Cxx(nb_frames_chunk, nb_bins, nb_channels,
                                   nb_channels);
            Cxx.setZero();

            // copy regularization into expanded form in middle of broadcast
            // loop
                ...
                for (int bin = 0; bin < nb_bins; ++bin)
                {
                    for (int source = 0; source < nb_sources; ++source)
                    {
                        ...
                        for (int ch1 = 0; ch1 < nb_channels; ++ch1)
                        {
                            for (int ch2 = 0; ch2 < nb_channels; ++ch2)
                            {
                                Cxx(frame, bin, ch1, ch2) +=
                                    regularization(ch1, ch2) +
                                    multiplier * R[source](bin, ch1, ch2);
                            }
                        }
                    }
                }
            ...
            // Invert Cxx
            std::cout << "\tInvert Cxx and Wiener gain calculation"
                      << std::endl;
            invert4D(
                Cxx); // Assuming invertMatrix performs element-wise inversion
            // Cxx is now inv_Cxx
            ...
            for (int source = 0; source < nb_sources; ++source)
            {
                ...
                Eigen::Tensor4dXcf gain(nb_frames_chunk, nb_bins, nb_channels,
                                        nb_channels);
                gain.setZero();
                    ...
                    for (int bin = 0; bin < nb_bins; ++bin)
                    {
                        for (int ch1 = 0; ch1 < nb_channels; ++ch1)
                        {
                            for (int ch2 = 0; ch2 < nb_channels; ++ch2)
                            {
                                ...
                                {
                                    gain(frame, bin, ch1, ch2) +=
                                        R[source](bin, ch1, ch3) *
                                        Cxx(frame, bin, ch3, ch2);
                                }
                            }
                        }
                    }
                ...
                for (int frame = 0; frame < nb_frames_chunk; ++frame)
                {
                    for (int bin = 0; bin < nb_bins; ++bin)
                    {
                        for (int ch1 = 0; ch1 < nb_channels; ++ch1)
                        {
                            ...
                            {
                                gain(frame, bin, ch1, ch2) *=
                                    v(frame + pos, bin, source);
                            }
                        }
                    }
                }

                std::cout << "\tApply gain to y, source: " << source
                          << ", pos: " << pos << std::endl;
                ...
                for (int frame = 0; frame < nb_frames_chunk; ++frame)
                {
                    for (int bin = 0; bin < nb_bins; ++bin)
                    {
                        for (int ch1 = 0; ch1 < nb_channels; ++ch1)
                        {
                            for (int ch2 = 0; ch2 < nb_channels; ++ch2)
                            {
                                std::complex<float> sample_cpx =
                                    y[source](ch2, frame + pos, bin);
                                std::complex<float> a =
                                    gain(frame, bin, ch2, ch1);
                                std::complex<float> b =
                                    mix_stft(ch1, frame + pos, bin);
                                y[source](ch2, frame + pos, bin) =
                                    sample_cpx + a * b;
                            }
                        }
        ...
        for (int bin = 0; bin < nb_bins; ++bin)
        {
            for (int ch1 = 0; ch1 < nb_channels; ++ch1)
            {
                for (int ch2 = 0; ch2 < nb_channels; ++ch2)
                {
                    ...
                    std::complex<float> b =
                        std::conj(y_j(ch2, frame + pos, bin));
                    ...
                    // Cj(frame, bin, ch1, ch2, 0) += (a_real*b_real -
                    // a_imag*b_imag); Cj(frame, bin, ch1, ch2, 1) +=
