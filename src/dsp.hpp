...
struct stft_buffers {
    ...
    stft_buffers(int n_samples) :
        ...
        pad_start(std::vector<float>(pad)),
        pad_end(std::vector<float>(pad)),
        padded_waveform_mono_in(std::vector<float>(n_samples + FFT_WINDOW_SIZE)),
        padded_waveform_mono_out(std::vector<float>(n_samples + FFT_WINDOW_SIZE)),
        windowed_waveform_mono(std::vector<float>(FFT_WINDOW_SIZE)),
        complex_spec_mono(std::vector<std::vector<std::complex<float>>>(nb_frames,
                                                                         std::vector<std::complex<float>>(nb_bins))),
    ...
    static std::vector<float> init_const_normalized_window(const std::vector<float> &window, int nb_frames) {
        float window_normalization_factor = FFT_WINDOW_SIZE + FFT_HOP_SIZE * (nb_frames - 1);
        std::vector<float> normalized_window = std::vector<float>(window_normalization_factor, 0.0f);
        ...
        for (int i = 0; i < nb_frames; ++i)
        {
            ...
            for (int j = sample; j < std::min((int)window_normalization_factor, sample + FFT_WINDOW_SIZE); ++j)
            {
