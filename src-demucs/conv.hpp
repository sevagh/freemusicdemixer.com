...
{
    return conv2d_gemm<in_channels, out_channels, kernel_height, kernel_width, stride_height, stride_width, pad_height, pad_width, dilation_height, dilation_width>(x, w, b);
}
...
Eigen::MatrixXf im2col_transposed(const Eigen::Tensor3dXf& input) {
    ...
    Eigen::MatrixXf output = Eigen::MatrixXf::Zero(expanded_height * expanded_width, channels * kernel_height * kernel_width);
            ...
            for (int kw = 0; kw < kernel_width; ++kw) {
                for (int h = 0; h < input_height; ++h) {
                    for (int w = 0; w < input_width; ++w) {
                        ...
                        // Check if the indices are within the bounds of the expanded output
                        if (expanded_h >= 0 && expanded_h < expanded_height && expanded_w >= 0 && expanded_w < expanded_width) {
                            int col_index = expanded_h * expanded_width + expanded_w;
...
template<int in_channels, int out_channels, int kernel_height, int kernel_width, int stride_height, int stride_width, int pad_height, int pad_width, int dilation_height, int dilation_width>
Eigen::Tensor3dXf conv2d_tr_gemm(const Eigen::Tensor3dXf &x, const Eigen::Tensor4dXf &w, const Eigen::Tensor1dXf &b) {
    int in_height = x.dimension(1);
    int in_width = x.dimension(2);

    // Calculate the output dimensions
    int out_height = (in_height - 1) * stride_height - 2 * pad_height + kernel_height;
    int out_width = (in_width - 1) * stride_width - 2 * pad_width + kernel_width;

    //demucscppdebug::debug_tensor_3dxf(x, "x input");
    // Apply an adapted im2col for transposed convolution
    Eigen::MatrixXf im2col_matrix = im2col_transposed<kernel_height, kernel_width, stride_height, stride_width, pad_height, pad_width, dilation_height, dilation_width>(x);
    ...
    // keeping in mind transpose weights are stored as (Cin, Cout, Kh, Kw) (not Cout, Cin, Kh, Kw)
    Eigen::Tensor4dXf w_swapped = w.shuffle(Eigen::array<int, 4>({1, 3, 2, 0})); // Note the change in the shuffle order
    Eigen::Tensor2dXf reshaped_weights_tensor = w_swapped.reshape(Eigen::array<int, 2>{out_channels, in_channels * kernel_height * kernel_width});
    Eigen::MatrixXf reshaped_weights = Eigen::Map<Eigen::MatrixXf>(reshaped_weights_tensor.data(), reshaped_weights_tensor.dimension(0), reshaped_weights_tensor.dimension(1));
    ...
    //demucscppdebug::debug_matrix_xf(result, "result conv2d-tr-gemm post-bias!");
    ...
    for (int ch = 0; ch < out_channels; ++ch) {
        for (int h = 0; h < out_height; ++h) {
            for (int w = 0; w < out_width; ++w) {
                // Calculate the linear index in the GEMM result corresponding to this output location
...
{
    return conv2d_tr_gemm<in_channels, out_channels, kernel_height, kernel_width, stride_height, stride_width, pad_height, pad_width, dilation_height, dilation_width>(x, w, b);
}
...
Eigen::Tensor3dXf conv1d_tr(const Eigen::Tensor3dXf &x, const Eigen::Tensor3dXf &w, const Eigen::Tensor1dXf &b)
{
    ...
    Eigen::Tensor3dXf y_out = conv2d_tr_gemm<in_channels, out_channels, kernel_size, 1, stride, 1, pad, 0, dilation, 1>(x_shuff, w_4d, b);
