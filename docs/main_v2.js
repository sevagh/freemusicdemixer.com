...
function hideOverlay() {
    ...
    document.getElementById('load-weights').style.display = 'none';
    document.getElementById('load-weights-2').style.display = 'none';
}

function showSpinner(appType) {
...
worker.onmessage = function(e) {
    if (e.data.msg === 'WASM_READY') {
        writeJsLog(`WASM module ${e.data.model} is now ready`)
        ...
        document.getElementById('load-progress-text').textContent = 'Loaded weights!';
        document.getElementById('load-progress-bar-2').style.width = '100%';
        document.getElementById('load-progress-text-2').textContent = 'Loaded weights!';
...
document.getElementById('load-weights').addEventListener('click', () => {
    ...
    document.getElementById('load-weights').disabled = true;
    document.getElementById('load-weights-2').disabled = true;
    worker.postMessage({ msg: 'LOAD_WASM', model: 'umx' });
});
...
document.getElementById('load-weights-2').addEventListener('click', () => {
    ...
    document.getElementById('load-weights').disabled = true;
    document.getElementById('load-weights-2').disabled = true;
    worker.postMessage({ msg: 'LOAD_WASM', model: 'demucs' });
});
